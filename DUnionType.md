...
> "Ah! It's like a compile time checked switch statement!" 
...
This library provides F# style ~discriminated~ unions for C#, using a custom type `DUnion<T0, ... Tn>`. An instance of this type holds a single value, which is one of the types in its generic argument list.
...
```csharp
public DUnion<User, InvalidName, NameTaken> CreateUser(string username)
{
...
{
    DUnion<User, InvalidName, NameTaken> createUserResult = CreateUser(username);
...
You can use also use `DUnion` as a parameter type, allowing a caller to pass different types without requiring additional overloads. This might not seem that useful for a single parameter, but if you have multiple parameters, the number of overloads required increases rapidly.

```csharp

public void SetBackground(DUnion<string, ColorName, Color> backgroundColor) { ... }
...
```csharp
   DUnion<string, ColorName, Color> backgroundColor = ...;
...
```csharp
   DUnion<string, DateTime> dateValue = ...;
   dateValue.Switch(
